...
This is where distance metrics $$ D(•) $$ come into play.  The difference we are trying to minimize can be calculated using different distance metrics. Different distance metrics can lead to different results. Now, let's run some tests under various simplified scenarios to see how each distance metric performs.
...
### Mixture with Brightness Difference
...
### A More Complicated Mixture

Now consider a more complicated scenarion. First we will consider larger number of spectral library. we will take 10 mineral from usgs spectra library, and take randomly betweeen 2 to 4 endmember to generate a mixture. As the selected mineral is unknown, the whole 10 mineral endmember will be considered during the unmixing. 
...
We will also add an gaussian noise, uniform noise,  gamma noise, poisson noise, and random constant additive noise as brightness modifier. To image below ilustrate the each noises:
...
Manhattan distance performed exceptionally well in this last case study, which probably can be attributed to its robustness to ouliers and more varying noise types. Despite this, its non-differentiable nature (recall the equation above) makes it less appealing for gradient-based optimization algorithms, especially for larger, more complex datasets. For this particular test, Manhattan distance takes 43 seconds for 1,000 data points, compared to 14 seconds for Euclidean distance and 11 seconds for angular distance.
